...
- 示例位姿很可能只是演示数据，不能直接套到当前箱体：
  ...
  - 四元数必须归一化。
- `add_bounding_box()`只过滤登记之后进入融合系统的新点，不会可靠地追溯删除此前已经融合进 TSDF/ESDF 的箱体体素。相关运行时分析见 [s1_add_bounding_box_runtime_analysis_20260831.md](../../assets/7b81546798fced5b--s1_add_bounding_box_runtime_analysis_20260831.md:18)。
...
当前仓库已有更稳健的 S1 实现，包含初始化检查、tag 冲突检查、失败回滚、显式确认和清理验证，可参考 [s1_navigation_cli.py](/home/lsy03/文档/ChatGPT/S1/experiments/20260827_s1_navigation/s1_navigation_cli.py:251) 和对应的 [使用说明](../../assets/097fe17e5ed31629--README.md:63)。其中固定的 4317 参数只适用于文档记录的特定抓取姿态，不能用于这里的示例箱体。
...
`add_bounding_box()`只过滤登记后的新输入点，不会自动清除已有 ESDF 体素。因此，补代码可以让流程更严格，但不能把当前不可达的 1 m 目标强行变为可达。相关证据在 [knowledge_base.md](../../assets/9532a89bcd51d69d--knowledge_base.md:617)。
...
具体运行链路见 [s1_add_bounding_box_runtime_analysis_20260831.md](../../assets/7b81546798fced5b--s1_add_bounding_box_runtime_analysis_20260831.md:19)。
