...
这一判断也与前两次只读核查一致：当前 ICP 不是可配置后端，现有代码还强制要求 `source=fdp`，而历史粗几何与 FDP 已表现出约 `19.5 mm` 平均 XY 差异，因此先建立可复现基线尤其必要。相关证据见[知识库](../../assets/9532a89bcd51d69d--knowledge_base.md:829)。
...
详细说明见 [README.md](../../assets/4e5683453ee99533--README.md:1)，结论已写入[知识库](../../assets/9532a89bcd51d69d--knowledge_base.md:942)。
...
12项测试全部通过；采集后控制锁已释放、生产容器仍停止、控制进程数为0。完整过程和哈希见[运行报告](../../assets/680d66f38aa4ceb9--RUN_20260904_LIVE_RGBD_SAM_LINGBOT.md:1)，持久结论已写入[知识库](../../assets/9532a89bcd51d69d--knowledge_base.md:975)。首次SAM3字段解析失败也已保留并记录，没有覆盖失败证据。SSH复用连接已关闭。
