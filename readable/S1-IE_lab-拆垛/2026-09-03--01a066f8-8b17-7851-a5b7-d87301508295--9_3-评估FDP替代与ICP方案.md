...
这一判断也与前两次只读核查一致：当前 ICP 不是可配置后端，现有代码还强制要求 `source=fdp`，而历史粗几何与 FDP 已表现出约 `19.5 mm` 平均 XY 差异，因此先建立可复现基线尤其必要。相关证据见[知识库](/home/lsy03/文档/ChatGPT/S1/docs/knowledge_base.md:829)。

## 5. 用户

FDP 基准脚本 给我放在这里 sftp://galbot@10.34.216.17/home/galbot/Lsy03/  是否可以？
你先讲讲我们FDP 原始调用层的实现步骤和目标，我们先计划再行动
...
详细说明见 [README.md](/home/lsy03/文档/ChatGPT/S1/experiments/20260903_fdp_baseline/README.md:1)，结论已写入[知识库](/home/lsy03/文档/ChatGPT/S1/docs/knowledge_base.md:942)。
...
这是一个 4×4 矩阵，表示箱体 mesh 相对于右腕相机的 6D 位姿：
...
- 从右腕相机坐标系转换到 `base_link`；
- 把 FDP mesh 坐标轴转换成箱体长、宽、高语义轴；
- 处理了箱体的180°对称方向；
- 记录了所有使用过的变换和质量检查。

最终输出格式对算法无关。以后 ICP 也应输出同一个字段：

```json
{
  "method": "foundationpose 或 constrained_icp",
  "object_model": "4317",
  "T_base_link_from_4317_semantic": [[...], [...], [...], [...]],
  "quality": {},
...
> 输入是冻结的右腕 RGB、深度、箱体 mask、相机参数和箱体模型；输出是4317箱体在 `base_link` 下坐标语义明确的6D位姿，供 FDP 与未来 ICP 做同标准比较。

## 24. 用户

给我简述一下这个输入是什么，输出是什么？ 是否和源代码 ./run_s1_mode00_one_click.py 一模一样？
...
12项测试全部通过；采集后控制锁已释放、生产容器仍停止、控制进程数为0。完整过程和哈希见[运行报告](/home/lsy03/文档/ChatGPT/S1/experiments/20260903_fdp_baseline/RUN_20260904_LIVE_RGBD_SAM_LINGBOT.md:1)，持久结论已写入[知识库](/home/lsy03/文档/ChatGPT/S1/docs/knowledge_base.md:975)。首次SAM3字段解析失败也已保留并记录，没有覆盖失败证据。SSH复用连接已关闭。
